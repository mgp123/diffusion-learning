...
class VarianceBlock(nn.Module):
    ...
    def forward(self, x, v):
        ...
        scale, bias = scale.unsqueeze(-1).unsqueeze(-1), bias.unsqueeze(-1).unsqueeze(
            -1
        )
        x = nn.functional.group_norm(x, 2)
        x = x * scale + bias
...
class SelfAttentionBlock(nn.Module):
    ...
    def forward(self, x):
        ...
        pos_embedding = self.positional_embedding(
            torch.arange(0, height * width, device=x.device)
        )
        ...
        attention = torch.bmm(q, k) / torch.sqrt(
            torch.tensor(channels, device=x.device)
        )
...
class DiffusionUnet(nn.Module):
    def __init__(
        ...
        for i in range(blocks):
            ...
            self.decoder_blocks.append(
                DecoderBlock(out_channels * 2, in_channels)
            )  # times 2 because of the skip connection cat
            ...
            out_channels *= (
                channel_multiplier[i + 1] if i + 1 < len(channel_multiplier) else 1
            )
    ...
    def sample(
        ...
        for t in tqdm(ts, leave=False):
            ...
            x_prev = (
                x
                - epsilon * scheudler.beta(t) * torch.sqrt(1 / scheudler.cumul_beta(t))
            ) / torch.sqrt(scheudler.alpha(t))
            
            x0 = (x - epsilon * torch.sqrt(scheudler.cumul_beta(t))) / torch.sqrt(
                scheudler.cumul_alpha(t)
            )
            ...
            if collect_latents:
                image_grid = torchvision.utils.make_grid(
                    x0, nrow=torch.sqrt(torch.tensor(x0.shape[0])).int()
                )
